...
CLR_ALIEN_L = (255, 220, 80)        # Sun Yellow
...
class GbaSfx:
    def __init__(self, enabled: bool = True):
        # Mixer init is handled by Game; we only check if it's available.
        self.enabled = enabled and (pygame.mixer.get_init() is not None)

    def _play_buf(self, buf: array.array):
        if not self.enabled:
            return
        try:
            # Convert mono -> stereo for better compatibility
...
class Game:
    def __init__(self):
        # Init mixer once, before the rest of pygame, so settings stick.
        pygame.mixer.pre_init(frequency=44100, size=-16, channels=2, buffer=512)
        pygame.init()

        # Try to bring up audio; if it fails, we keep running silently.
        audio_enabled = True
        try:
        ...
        self.sfx = GbaSfx(enabled=audio_enabled)
        ...
        self.score = 0
        self.lives = 3
        self.alien_fire_chance = 0.01  # base difficulty; increases by level
    ...
    def update(self):
        if self.state == "PLAY":
            ...
            # Alien shooting (arcade-correct-ish: only bottom-most alien per column can fire)
            if self.aliens and random.random() < self.alien_fire_chance:
                columns = {}
                for alien in self.aliens:
                    col_key = alien.rect.x  # all aliens in a column share x
    ...
    def draw(self):
        ...
        if self.state == "MENU":
            ...
            instructions = [
                ...
                "",
                "Aliens move faster as there",
                "are fewer of them.",
                "",
                "Avoid enemy fire and prevent",
                "aliens from reaching the bottom.",
            ]
            ...
            credits = [
                ...
                "AC Computing Gaming Division",
                "",
                "Sound Design:",
                "GBA Style SFX Synthesizer",
                "",
                "Special Thanks:",
                "Space Invaders (1978)",
                "Famicom/NES Developers",
                "",
                "[C] 1999-2026",
            ]
            ...
            about_text = [
                ...
                "inspired by classic arcade",
                "games from the Famicom/NES era.",
                "",
                "Features authentic 8-bit style",
                "graphics and GBA-inspired",
                "sound effects.",
                "",
                "Created with PyGame",
            ]
